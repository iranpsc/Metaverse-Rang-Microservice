...
func TestBuildingRepository_FindBuildingModelByModelID(t *testing.T) {
	...
	t.Run("return nil when not found", func(t *testing.T) {
		model, err := repo.FindBuildingModelByModelID(ctx, "999999")
		require.NoError(t, err)
		assert.Nil(t, model)
	})

	t.Run("reject non numeric model id", func(t *testing.T) {
		_, err := repo.FindBuildingModelByModelID(ctx, "nonexistent_model")
		require.Error(t, err)
	})
...
func TestBuildingRepository_DeleteBuilding(t *testing.T) {
	...
	t.Run("delete building by feature_id and model_id", func(t *testing.T) {
		...
		// Delete building
		_, err = repo.DeleteBuilding(ctx, featureID, modelIDStr)
		require.NoError(t, err)
