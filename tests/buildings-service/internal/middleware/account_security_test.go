...
func TestAccountSecurityMiddleware_AllowsWhenUnlocked(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/features/1/build/1001", nil)
...
func TestAccountSecurityMiddleware_Returns410WhenLocked(t *testing.T) {
	...
	for _, method := range []string{http.MethodPost, http.MethodPut, http.MethodDelete} {
		t.Run(method, func(t *testing.T) {
			called = false
			req := httptest.NewRequest(method, "/api/features/1/build/buildings/23", nil)
			req.Header.Set("Authorization", "Bearer token")
			rr := httptest.NewRecorder()
			handler.ServeHTTP(rr, req)
...
func TestAccountSecurityMiddleware_SkipsSafeMethods(t *testing.T) {
	...
	handler := middleware.AccountSecurityMiddleware(auth)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = context.WithValue(r.Context(), authpkg.UserContextKey{}, &authpkg.UserContext{UserID: 1})
		w.WriteHeader(http.StatusOK)
	}))

	rr := httptest.NewRecorder()
	handler.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/api/features/buildings/completed", nil))
