...
func TestOptionalAuthMiddleware(t *testing.T) {
	t.Run("no token continues", func(t *testing.T) {
		called := false
		handler := middleware.OptionalAuthMiddleware(&mockAuthClient{})(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			called = true
			w.WriteHeader(http.StatusOK)
		}))
		rr := httptest.NewRecorder()
	...
	t.Run("valid token injects user", func(t *testing.T) {
		handler := middleware.OptionalAuthMiddleware(&mockAuthClient{})(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			user, err := middleware.GetUserFromRequest(r)
			if err != nil || user.UserID != 42 {
				t.Fatal(err, user)
			}
