// Package repository provides data access for the buildings service.
package repository
...
// parseExactModelID accepts only a full decimal 3D model id. Prefixes such as "123abc" are rejected.
func parseExactModelID(modelID string) (uint64, error) {
	modelID = strings.TrimSpace(modelID)
	if modelID == "" {
		return 0, fmt.Errorf("model_id must be numeric")
	}
	id, err := strconv.ParseUint(modelID, 10, 64)
	if err != nil {
		return 0, fmt.Errorf("model_id must be numeric: %w", err)
	}
	return id, nil
}

// FindBuildingModelByModelID finds a building model by its 3D API model_id.
func (r *BuildingRepository) FindBuildingModelByModelID(ctx context.Context, modelID string) (*pb.BuildingModel, error) {
	dbModelID, err := parseExactModelID(modelID)
	if err != nil {
		return nil, err
	}
...
// withFeatureLock serializes build and destroy for one feature on a single connection.
// The named lock is released on that connection after the transaction commits.
func withFeatureLock[T any](ctx context.Context, db *sql.DB, featureID uint64, fn func(context.Context, *sql.Tx) (T, error)) (T, error) {
	var zero T
	lockName := fmt.Sprintf("buildings:feature:%d", featureID)
	conn, err := db.Conn(ctx)
	if err != nil {
		return zero, fmt.Errorf("lock building feature: %w", err)
	}
	...
	if locked.Int64 != 1 {
		return zero, fmt.Errorf("timed out locking building feature")
	}
	defer func() {
	...
	if err != nil {
		return zero, fmt.Errorf("begin building transaction: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if err := tx.Commit(); err != nil {
		return zero, fmt.Errorf("commit building transaction: %w", err)
	}
	committed = true
	return result, nil
}

// CreateBuilding inserts a building while holding a per-feature lock so two
// concurrent builds cannot both pass the existence check.
func (r *BuildingRepository) CreateBuilding(ctx context.Context, featureID, userID uint64, buildingModelID string, launchedSatisfaction, rotation, position, information string, constructionStartDate, constructionEndDate time.Time, bubbleDiameter float64) error {
	modelNum, err := parseExactModelID(buildingModelID)
	if err != nil {
		return err
	}
	_, err = withFeatureLock(ctx, r.db, featureID, func(ctx context.Context, tx *sql.Tx) (struct{}, error) {
		var count int
		if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM buildings WHERE feature_id = ?`, featureID).Scan(&count); err != nil {
			return struct{}{}, fmt.Errorf("failed to check building existence: %w", err)
		}
		if count > 0 {
			return struct{}{}, fmt.Errorf("feature already has a building")
		}

		var localID uint64
		err := tx.QueryRowContext(ctx, `SELECT id FROM building_models WHERE model_id = ?`, modelNum).Scan(&localID)
		if err == sql.ErrNoRows {
			return struct{}{}, fmt.Errorf("building model not found: %s", buildingModelID)
		}
		if err != nil {
			return struct{}{}, fmt.Errorf("failed to find building model: %w", err)
		}
		...
		_, err = tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
		`, featureID, userID, localID, constructionStartDate, constructionEndDate,
			launchedSatisfaction, information, rotation, position, bubbleDiameter)
		if err != nil {
			return struct{}{}, fmt.Errorf("failed to create building: %w", err)
		}
		return struct{}{}, nil
	})
	return err
...
func (r *BuildingRepository) FindByFeatureID(ctx context.Context, featureID uint64) ([]*pb.Building, error) {
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("failed iterating buildings: %w", err)
	}
...
func (r *BuildingRepository) UpdateBuilding(ctx context.Context, featureID uint64, buildingModelID string, launchedSatisfaction, rotation, position, information string, constructionEndDate time.Time, bubbleDiameter float64) (*pb.Building, error) {
	...
	updated, err := r.FindBuildingByFeatureAndModel(ctx, featureID, buildingModelID)
	if err != nil {
		return nil, err
	}
	if updated == nil {
...
// DeleteBuilding removes a building under a per-feature lock and returns the
// launched_satisfaction that was stored on the deleted row.
func (r *BuildingRepository) DeleteBuilding(ctx context.Context, featureID uint64, buildingModelID string) (string, error) {
	modelNum, err := parseExactModelID(buildingModelID)
	if err != nil {
		return "", err
	}
	return withFeatureLock(ctx, r.db, featureID, func(ctx context.Context, tx *sql.Tx) (string, error) {
		var localID uint64
		err := tx.QueryRowContext(ctx, `SELECT id FROM building_models WHERE model_id = ?`, modelNum).Scan(&localID)
		if err == sql.ErrNoRows {
			return "", fmt.Errorf("building model not found: %s", buildingModelID)
		...
		err = tx.QueryRowContext(ctx, `
			SELECT launched_satisfaction
			FROM buildings
			WHERE feature_id = ? AND model_id = ?
			FOR UPDATE
		`, featureID, localID).Scan(&launched)
		if err == sql.ErrNoRows {
		...
		result, err := tx.ExecContext(ctx, `DELETE FROM buildings WHERE feature_id = ? AND model_id = ?`, featureID, localID)
		if err != nil {
